...
class SegTraqer:
    ...
    def run_baseline(self, inplace=True):
        ...
        gpc = bl.genes_per_cell(self.sdata, self.points_key, self.cell_key_tables, self.gene_key).set_index(
            self.cell_key_tables
        )
        tpc = bl.transcripts_per_cell(self.sdata, self.points_key, self.cell_key_tables).set_index(self.cell_key_tables)
        dens = bl.transcript_density(
            self.sdata, self.table_key, self.points_key, self.cell_key_tables, self.area_key
