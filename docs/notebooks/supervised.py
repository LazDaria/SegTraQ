...
# - `contamination_strength`: the fraction of transcripts assigned to the cell that correspond to contamination-relevant genes, computed as `contamination_counts` / `total_transcript_counts`.
#
...
# - `contamination_matrix`: the fraction of target cells of type `c_tgt` that contain at least one contamination-relevant transcript originating from source cell type `c_src`, and
# - `contamination_strength_matrix`: the mean contamination strength of target cells of type `c_tgt` attributable to source cell type `c_src`, computed by averaging the source-specific contamination strength across all evaluable target cells.
# - `contamination_evaluable_cells_matrix` : Directed source-to-target matrix. Entry (c_src, c_tgt) is the number of target cells for which contamination from source type c_src could be evaluated.
#
# If no neighborhood graph is present (`neighbors_key not in sdata.tables[tables_key].obsp`), it is automatically computed from cell centroids using a Delaunay graph.
...
# %% [markdown]
# The spatial plot below shows, for each target cell, the number of contaminating transcripts. If desired, this quantity can be normalized by the total number of transcripts per cell. As expected, the plot correlates with the `negative_marker_avoidance`score from above.
...
#
# The bubble plot of the `contamination_strength_matrix` provides complementary information by quantifying how large the contamination signal is when it occurs. For example, although myoepithelial and DCIS2 cells are frequently neighbors, contamination from DCIS2 into myoepithelial cells is negligible. Moreover, relatively few myoepithelial cells are evaluable for DCIS2 despite their spatial proximity because the two cell types share only a small number of mutually exclusive markers (four in this example; see `overlap_df`), and these markers may not be expressed in the neighboring DCIS2 cells in every local neighborhood. 
#
...
# 2) **Spatial proximity**, i.e. how frequently the two cell types occur as neighbors. The more often they are adjacent, the greater the opportunity for contamination.
# 3) **Expression similarity**, .ie. contamination between transcriptionally similar cell types is both less consequential and more difficult to detect. Because the two cell types share fewer mutually exclusive marker genes, transferred transcripts have less impact on the measured expression profile, and fewer informative genes are available for quantifying contamination.
